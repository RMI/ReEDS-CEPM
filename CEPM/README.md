...
| [`preprocessing/datacenter_load_forecast/`](preprocessing/datacenter_load_forecast/) | Notebook and EPRI Powering Intelligence export that build state-level data center load-site inputs for all 48 contiguous states in Low/Medium/High scenarios, written to `inputs/load/`. This is the load-site input the national CEPM cases currently select (`GSw_LoadSiteTrajectory`). |
